...
RUN pip install --no-cache-dir -r requirements.txt && \
  ...
  python -c "from os import getenv; from transformers import AutoTokenizer, AutoFeatureExtractor, Wav2Vec2ForCTC; AutoFeatureExtractor.from_pretrained(getenv('model')); tokenizer = AutoTokenizer.from_pretrained(getenv('model')); model = Wav2Vec2ForCTC.from_pretrained(getenv('model'))"

COPY --chown=wav2vec2:wav2vec2 main.py .
